...
using AdminDashboard.Core.Dtos.General;
...
namespace AdminDashboard.Core.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<GeneralServiceResponseDto> RegisterAsync(RegisterDto registerDto)
        {
            ...
            // Add a Default EMPLOYEE Role to all users
            await _userManager.AddToRoleAsync(newUser, StaticUserRoles.EMPLOYEE);
        ...
        public async Task<GeneralServiceResponseDto> SeedRolesAsync()
        {
            ...
            bool isProjectManaherRoleExists = await _roleManager.RoleExistsAsync(StaticUserRoles.PROJECTMANAGER);
            bool isHRManagerRoleExists = await _roleManager.RoleExistsAsync(StaticUserRoles.HRMANAGER);
            bool isEmployeeRoleExists = await _roleManager.RoleExistsAsync(StaticUserRoles.EMPLOYEE);

            if (isAdminRoleExists && isProjectManaherRoleExists && isHRManagerRoleExists && isEmployeeRoleExists)
                return new GeneralServiceResponseDto()
                {
                    IsSucceed = true,
                    StatusCode = 200,
            ...
            await _roleManager.CreateAsync(new IdentityRole(StaticUserRoles.HRMANAGER));
            await _roleManager.CreateAsync(new IdentityRole(StaticUserRoles.EMPLOYEE));
        ...
        public async Task<GeneralServiceResponseDto> UpdateRoleAsync(ClaimsPrincipal User, UpdateRoleDto updateRoleDto)
        {
            ...
            {
                if (updateRoleDto.NewRole == RoleType.EMPLOYEE || updateRoleDto.NewRole == RoleType.PROJECTMANAGER || updateRoleDto.NewRole == RoleType.HRMANAGER)
                {
                    // admin can change the role of everyone except for admins
